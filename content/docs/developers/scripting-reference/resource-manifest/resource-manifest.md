...
The **resource manifest** is a file named `fxmanifest.lua` (or previously, `__resource.lua`), placed in a [resource folder](/docs/developers/scripting-manual/introduction/introduction-to-resources) on the server.
...
Sets the resource's [NUI]({{%  ref "/docs/developers/scripting-manual/nui-development/full-screen-nui.md"  %}}) page to the defined file or URL. If specifying a a file, the file (along with its dependencies) has to be referenced using [files](#file "wikilink").
